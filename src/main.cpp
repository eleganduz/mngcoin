...
#if defined(NDEBUG)
# error "Mngcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x201e6a2d12c18566953816314abe893899b6a0d679ac7823864d7dcf49b133d3");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Mngcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Mngcoin Signed Message:\n";
...
{
    // Mngcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // Mngcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Mngcoin: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // Mngcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Mngcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Mngcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x01bcdffe86bbf02caefd65d13a24e752ec860edd1c9512d7b1b701a696f6b35f");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "25/11/2021 MNGCOIN NEW COIN MANGU.UZ";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 1 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("045f426015d0b442ac65bb203c63324801056ad3afa99bb9c494564216ecbc64b58349ea1bbe4677832c679807998dce88949ec7b113cbf31c36752287c8b5ded9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1637843161;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086260935;

        if (fTestNet)
        {
            block.nTime    = 1637843126;
            block.nNonce   = 388100318;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x2b4b21440155edb936812bbdae3e23202bb1d2ae9819a0f35cbe034b3486fc07"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc2, 0xb5, 0xdc }; // Mngcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MngcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MngcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MngcoinMiner(CWallet *pwallet)
{
    printf("MngcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("mngcoin-miner");
    ...
    try { loop {
        ...
        printf("Running MngcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MngcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MngcoinMiner, pwallet));
}
